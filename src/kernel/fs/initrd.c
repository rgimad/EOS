...
void initrd_init(uint32_t phys_begin, uint32_t phys_end)
{
	...
	physical_addr frame;
	virtual_addr virt;
	for (frame = PAGE_ALIGN_DOWN(phys_begin), virt = PAGE_ALIGN_DOWN(initrd_begin) + PAGE_SIZE; frame <= (PAGE_ALIGN_DOWN(phys_end)); frame += PAGE_SIZE, virt += PAGE_SIZE)
    {
