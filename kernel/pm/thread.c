...
thread_t *create_user_thread(process_t *parent_proc, void *entry_point) {
    ...
    new_uthread->registers.eip = (uint32_t) entry_point; // Set up the instruction pointer to given entry point
...
thread_t *create_kernel_thread(void *entry_point) { // TODO: somewhere in this function kernel crashes
    asm volatile("cli");
