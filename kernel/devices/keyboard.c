...
static kbd_scancode uppercase_scancodes[] = {
    // Numerical keys
    { 0x02, '1' },
    { 0x03, '2' },
    { 0x04, '3' },
...
static kbd_scancode shift_modified_scancodes[] = {
    // Numerical keys
    { 0x02, '!' },
    { 0x03, '@' },
    { 0x04, '#' },
...
{
    // qemu_printf("keyboard_buffer_push() start\n");
    ...
    if (kbd_buf_in - kbd_buf_out > 256) {
        // qemu_printf("kbd buf overflow\n");
        //buffer overflow happened
    }
    // qemu_printf("kbd_buf_in = %u, kbd_buf_out = %u\n", kbd_buf_in, kbd_buf_out);
    // qemu_printf("keyboard_buffer_push() end\n");
}

// Read last keyboard event from buffer
kbd_event keyboard_buffer_pop()
{
    // qemu_printf("keyboard_buffer_pop()\n");
...
{
    // qemu_printf("keyboard_wait_irq() start\n");
    while (!kbd_irq_fired) {
        // qemu_printf("waiting...\n");
    }
    kbd_irq_fired = false;
...
void keyboard_handler(__attribute__((unused)) struct regs *r)
{
    ...
    switch (scancode) {
    case KEY_LSHIFT:
        state.lshift = !state.release;
        break;
    case KEY_RSHIFT:
        state.rshift = !state.release;
        break;
    case KEY_LCTRL:
        state.lctrl = !state.release;
        break;
    case KEY_RCTRL:
        state.rctrl = !state.release;
        break;
    case 0xE0:
        state.code |= 0x100;
